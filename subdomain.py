    ...
    if select == "1":
        def cut(text='', leng=False):
            ...
            def revip(target):
                try:
                    getdom = requests.get(
                        "https://sonar.omnisint.io/reverse/" + target)
            ...
            target = open(input(Fore.WHITE+'List:~# '), 'r').read().replace(
                    'http://', '').replace('https://', '').splitlines()
        ...
        def revip(target):
            try:
                getdom = requests.get("https://sonar.omnisint.io/reverse/" +
                                    target and "https://sonar.omnisint.io/subdomains/" + target)
                if "null" in getdom.text:
                    print("[?] "+cut(target, 15)+" {}=> ".format(Fore.WHITE) + "{}Maybe Domains Down".format(Fore.RED))
                else:
                    result = json.loads(getdom.text)
        ...
        print("===============================")
        os.system('dir' if os.name == 'nt' else 'ls')
        target = open(input(Fore.WHITE+'List:~# '), 'r').read().replace('http://',
                    '').replace('https://', '').splitlines()
        Thread = input(Fore.WHITE+'Thread:~# ')
        pool = ThreadPool(int(Thread))
        pool.map(revip, target)
        pool.close()
